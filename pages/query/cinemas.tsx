...
const Cinemas: NextPage = ({data}:any) => {
    ...
    const getCinemas = async()=>{
        await axiosInstance.get(`api/controller/cinema/${query}`).then((res)=>{
    ...
    <div className="news-area section-padding-100-70">
        <div className="container">
            <div className="row">
                ...
                {NewCinema.length > 0?  <h6>Search for &nbsp; &apos;{query}&apos;</h6> : <h6>No search result for cinema {query}</h6>} 
                </div>
                    ...
                    <div className="single-blog-post d-flex flex-wrap style-5 mb-30" key={i}>
                        ...
                        <Link href={`/query/${data?._id}`}><a ><img src={data?.image} alt="" style={{width:'100%',height:'130px',objectFit:'cover'}}/></a></Link>
                        </div>
                        <div className="blog-content" style={{marginTop:'1rem'}}>
                        <Link href={`/query/${data?._id}`}><a className="post-title">{data?.name}</a></Link>
                            <p>{data?.address}</p>
                        </div>
                    </div>
                    ))}
